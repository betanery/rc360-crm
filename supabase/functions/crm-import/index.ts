  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const data = {
        ...
        market_time: String(row.market_time || row.tempo_mercado || "").trim() || null,
        team_size: String(row.team_size || row.tamanho_equipe || "").trim() || null,
        referred_by: String(row.referred_by || row.indicado_por || "").trim() || null,
        main_pain: String(row.main_pain || row.maior_dor || "").trim() || null,
        wants_feedback: String(row.wants_feedback || row.quer_devolutiva || "").trim() || null,
